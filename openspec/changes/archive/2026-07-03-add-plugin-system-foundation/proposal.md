...
- Fixes the **trust model for the MVP**: no sandbox (draw.io precedent) — a plugin is a local JS file the user explicitly loads; that act is informed consent. Plugins must never touch the Zustand store, `IStoragePort`, or React Flow internals directly — only the versioned `StructuraPlugin.*` API. The manifest still *declares* permissions so a future sandboxed/marketplace model can be introduced without breaking the manifest format.
