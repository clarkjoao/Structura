/**
 * Passo 1 / Passo 4 — the drag-commit loop, measured to silence.
 *
 * Drags a leaf node ~200px, releases, and waits until 3s pass with no long task.
 * Counts every `structura:recentDiagrams` write in that window and records whether
 * `structura_diagram-store` was written (i.e. whether the diagram actually saved).
 *
 * Usage: node probe-loop.mjs <G|M2> <label> <out.json> [reps]
 */
...
const MOVES = 50;      // 50 × 4px = 200px
const STEP = 4;
const SILENCE = 3000;  // spec: silence = 3s with no long task
const CAP = 120000;
...
async function freshPage() {
  ...
  await page.evaluate(() => {
    ...
    window.__arm = () => {
      window.__w = []; window.__L = [];
      if (!window.__orig) window.__orig = Storage.prototype.setItem;
      Storage.prototype.setItem = function (k, v) {
        window.__w.push({ t: +performance.now().toFixed(0), k, bytes: typeof v === "string" ? v.length : -1 });
        return window.__orig.call(this, k, v);
      };
    ...
    window.__disarm = () => {
      if (window.__orig) Storage.prototype.setItem = window.__orig;
      window.__lo?.disconnect?.();
      return { w: window.__w || [], L: window.__L || [] };
    };
  });
...
async function findLeaf() {
  return page.evaluate(() => {
    ...
    for (const n of ns) {
      ...
      if (r.left < 60 || r.top < 110 || r.right > innerWidth - 460 || r.bottom > innerHeight - 190) continue;
...
for (let r = 0; r < REPS; r++) {
  ...
  try {
    ...
    for (let i = 1; i <= MOVES; i++) { await page.mouse.move(t.x + STEP * i, t.y + Math.sin(i / 5) * 10, { steps: 1 }); await page.waitForTimeout(16); }
    await page.evaluate(() => new Promise((res) => requestAnimationFrame(() => requestAnimationFrame(res))));
    await page.evaluate(() => window.__arm());
    const tUp = Date.now();
    await page.mouse.up();
    const quietMs = await page.evaluate(({ s, c }) => window.__quiet(s, c), { s: SILENCE, c: CAP });
    rep.wallMs = Date.now() - tUp;
    // wall time to silence, minus the silence window itself
    rep.settleMs = Math.max(0, rep.wallMs - SILENCE);
    rep.hitCap = quietMs >= CAP;
    const { w, L } = await page.evaluate(() => window.__disarm());
    const byKey = {};
    for (const x of w) { byKey[x.k] = byKey[x.k] || { n: 0, bytes: 0 }; byKey[x.k].n++; byKey[x.k].bytes += x.bytes; }
    rep.recentWrites = byKey["structura:recentDiagrams"]?.n ?? 0;
    rep.storeWritten = Boolean(byKey["structura_diagram-store"]);
    rep.byKey = byKey;
    rep.longCount = L.length;
    rep.longTotalMs = +L.reduce((a, b) => a + b, 0).toFixed(0);
    rep.longMaxMs = L.length ? Math.max(...L) : 0;
    // verify the drag actually persisted the new position
    rep.persistedMove = await page.evaluate((nid) => {
      const raw = localStorage.getItem("structura_diagram-store");
      if (!raw) return null;
      const s = JSON.parse(raw).state;
      const d = s.diagrams[s.activeDiagramId];
      const l = d?.nodeLayouts?.[nid];
      return l ? { x: Math.round(l.x), y: Math.round(l.y) } : null;
    }, t.id);
  } catch (e) { rep.error = String(e).slice(0, 200); }
  reps.push(rep);
  console.log(`  rep${r} recentWrites=${rep.recentWrites} settle=${rep.settleMs}ms long=${rep.longCount}/${rep.longTotalMs}ms storeWritten=${rep.storeWritten} hitCap=${rep.hitCap} pos=${JSON.stringify(rep.persistedMove)}`);
}

const summary = {
  label: LABEL, fixture: FIXTURE, reps: REPS,
  domNodes: reps.find((r) => r.domNodes)?.domNodes ?? null,
  recentWrites: stat(reps.map((x) => x.recentWrites)),
  settleMs: stat(reps.map((x) => x.settleMs)),
  longCount: stat(reps.map((x) => x.longCount)),
  longTotalMs: stat(reps.map((x) => x.longTotalMs)),
  storeWrittenAll: reps.every((r) => r.storeWritten === true),
  storeWrittenCount: reps.filter((r) => r.storeWritten === true).length,
  hitCapCount: reps.filter((r) => r.hitCap === true).length,
};
console.log("\nSUMMARY " + LABEL);
