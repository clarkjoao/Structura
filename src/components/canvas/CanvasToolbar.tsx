...
const c4Options: { type: C4ElementType; label: string; icon: typeof Network }[] = [
...
const CanvasToolbar = () => {
  const { activeView, navigateUp, addElement } = useModel();
  ...
  const handleAddC4 = (type: C4ElementType) => {
  ...
  const handleAddAws = (categoryId: AwsCategoryId, serviceId: string, serviceName: string) => {
    addElement(categoryId, serviceName, activeView.rootElementId, { x: 300, y: 200 }, serviceId);
    setShowAws(false);
    setShowAdd(false);
    setExpandedCat(null);
  };
  ...
  return (
    <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
      ...
      <div className="relative">
        <button
          onClick={() => { setShowAdd(!showAdd); setShowAws(false); }}
        ...
        {showAdd && (
          <div className="absolute top-full left-0 mt-1 rounded-lg border border-border bg-card shadow-xl py-1 min-w-[200px]">
            ...
            <div className="px-3 py-1">
              <span className="text-[9px] font-mono uppercase tracking-wider text-muted-foreground">C4 Model</span>
            </div>
            {c4Options.map((opt) => (
              <button
                key={opt.type}
