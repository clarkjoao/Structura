 ...
 * for one named by id or read off disk. Two names a letter apart for the same
 * job. This is the one route, and it kept the name that is already out in the
 * world: `generateViewerUrl` writes `/viewer#data=` into every shared link, and
 * `EmbedModal` writes `/viewer` into every iframe snippet it hands out.
 *
 * Diagrams are rendered as authored. A diagram that arrives with its positions
 * was arranged by whoever is sharing it, and re-arranging it would replace the
 * picture they are sharing with a different one. So all paths — hash,
 * `postMessage`, `?diagramId`, and `?source=file` — render what they were
 * given.
 *
...
function FileSource({ path }: { path: string | null }) {
  ...
  if (state.status !== "ready") return <ViewerLoading label={t("embedPage.loading")} />;

  return <ViewerCanvas diagram={state.diagram} showOpenInStructuraButton={false} />;
}

/** `?diagramId=` — one of the reader's own diagrams. */
