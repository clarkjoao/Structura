 ...
 * did not. This hashes the synced surface so a mismatch is detectable in ~16
 * bytes, and the expensive repair (a full snapshot) only runs when one shows up.
 *
 * The server computes the same value over its own snapshot. Any change here
 * must be mirrored in `server/src/snapshotChecksum.ts`, which
 * `snapshotChecksum.parity.test.ts` pins to this implementation.
 */
