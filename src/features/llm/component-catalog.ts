...
export function buildComponentTypeCatalog(): string {
  const sections: string[] = [
    ...
    "Prefer a search-only response first; wait for Catalog tool results in the next turn before add_node.",
    "If you emit search_elements and add_node in the same response, every cloud add_node must use an",
    "exact elementType+serviceId pair returned by that same search — unmatched add_node calls are skipped.",
