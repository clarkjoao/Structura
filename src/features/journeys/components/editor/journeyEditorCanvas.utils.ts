...
export function createJourneyEditorNodeBuildContext(
  diagram: Diagram,
...
export function buildJourneyEditorNodes(
  diagram: Diagram,
  selectedComponentId: string | null,
  allDiagrams: Record<string, Diagram>,
  flowVisuals: JourneyEditorCanvasFlowVisuals | null = null,
): Node[] {
...
export function buildJourneyEditorEdges(
  diagram: Diagram,
  flowVisuals: JourneyEditorCanvasFlowVisuals | null = null,
): Edge[] {
  const snapshot = getCachedCanvasSnapshot(diagram);
