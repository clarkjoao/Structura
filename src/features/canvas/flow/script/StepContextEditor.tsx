...
import { fromSetRows, newSetRow, setsFromPayload, toSetRows, type SetRow } from "./stepContext";

/**
 * Writing what a step introduces, consumes and expects — against the state it
 * is being written into.
 *
 * The scope comes first and is read-only: what the steps before this one have
 * already set, folded exactly as the reading folds it, so the panel someone
 * authors against is the panel they will read. Without it every key here was
 * typed from memory, and a `reads` that named a key nothing sets looked no
 * different from one that named a key something does.
 */

interface Props {
  step: FlowStep;
  /** Everything set before this step, innermost call first, with where it came from. */
...
export function StepContextEditor({ step, scope, onChange }: Props) {
  ...
  const readable = [...scope.map((entry) => entry.key)];
    ...
    <div className="flex flex-col gap-2 pt-1" onClick={(event) => event.stopPropagation()}>
      {scope.length > 0 && (
        <div className="flex flex-col gap-1">
          ...
          <div
            ...
            className="flex flex-col gap-px rounded border border-border bg-secondary/50 px-1.5 py-1"
          >
            {scope.map((entry) => (
      ...
      <div className="flex flex-col gap-1">
        ...
        {rows.map((row, index) => {
          ...
          const shadowed = scope.find((entry) => entry.key === row.key.trim());
          return (
          ...
          {readable.map((key) => {
            ...
            return (
              <button
                key={key}
                type="button"
                data-testid="step-context-read-chip"
                aria-pressed={on}
