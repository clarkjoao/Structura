...
import { X, GripVertical, Copy, Check, GitBranch } from "lucide-react";
import { toast } from "sonner";
import { useTranslation } from "react-i18next";
import type { ActiveBranch, BranchOwnerInfo } from "./RecordingModeContext";
...
const FlowRecorderPanel = ({
  ...
  const handleConditionFormSubmit = () => {
    ...
    if (conditionForm.index === -1) {
      // Adding a brand new condition step
      onAddConditionStep(conditionForm.label, validBranches);
      setExpandedStep(steps.length); // will be the last step after add
    } else {
      onConvertStepToCondition(conditionForm.index, conditionForm.label, validBranches);
      setExpandedStep(conditionForm.index);
    }
    setConditionForm(null);
  };

  // Build a temporary Flow object for mermaid preview
  const mermaidPreview = useMemo(() => {
    const stepsRecord: Record<string, FlowStep> = {};
    for (const s of steps) stepsRecord[s.id] = s;
    // Link sequentially (non-condition steps)
    for (let i = 0; i < steps.length - 1; i++) {
      if (steps[i].type !== 'condition' && !steps[i].next) {
        stepsRecord[steps[i].id] = { ...stepsRecord[steps[i].id], next: steps[i + 1].id };
      }
    }
    const tempFlow: Flow = {
      id: "preview",
  ...
  }, [steps, components, connections, name]);
  ...
  return (
    <div className="w-80 h-full min-h-0 border-l border-border bg-card overflow-hidden flex flex-col">
      ...
      {activeBranch && (
        <div className="px-3 py-2 border-b border-amber-400/30 bg-amber-500/5 flex items-center justify-between">
          <span className="text-[10px] font-semibold text-amber-400">
            {t("flowRecorder.recordingBranch", { label: activeBranch.branchLabel, defaultValue: `Recording: ${activeBranch.branchLabel} →` })}
          </span>
          <button
            onClick={onExitBranch}
            className="text-[10px] font-medium text-muted-foreground hover:text-foreground border border-border rounded px-2 py-0.5 transition-colors"
          >
            {t("flowRecorder.exitBranch", "Exit branch")}
          </button>
        </div>
      )}
      ...
      <div className="p-3 space-y-3 flex-1 min-h-0 overflow-y-auto">
              ...
              {steps.map((step, i) => {
                ...
                <div key={step.id}>
                  <div
                    ...
                    } ${step.type === 'condition' ? "border-l-2 border-amber-400" : ""} ${isBranchStep ? "ml-3 border-l border-amber-400/30" : ""} ${dragIdx === i ? "opacity-40" : ""} ${
                      ...
                      {step.type === 'condition' ? (
                        ...
                        <>
                          ...
                          <div className="space-y-0.5">
                            <p className="text-[9px] text-muted-foreground font-semibold uppercase">{t("flowRecorder.branches", "Branches")}</p>
                            {step.branches?.map((branch, bi) => {
                              const branchStepCount = steps.filter((s) => {
                                const info = branchOwnership.get(s.id);
                                return info && info.conditionStepId === step.id && info.branchIndex === bi;
                              }).length;
                              const isActiveBranch = activeBranch?.conditionStepId === step.id && activeBranch?.branchIndex === bi;
                              return (
                              <div key={bi} className="space-y-0.5">
                                <div className="flex items-center gap-1">
                                  <input
                                    value={branch.label}
                                    onChange={(e) => onUpdateBranchLabel(i, bi, e.target.value)}
                                  ...
                                  />
                                  <button
                                    onClick={(e) => { e.stopPropagation(); onEnterBranch(i, bi); }}
                                    className={`text-[9px] font-medium px-1.5 py-0.5 rounded transition-colors shrink-0 ${
                                      isActiveBranch
                                        ? "bg-amber-500 text-white"
                                        : "text-amber-400 hover:bg-amber-500/10"
                                    }`}
                                  >
                                    {isActiveBranch ? t("flowRecorder.recording", "REC") : t("flowRecorder.record", "Record")}
                                  </button>
                                  {(step.branches?.length ?? 0) > 2 && (
                                    <button
                                      onClick={(e) => { e.stopPropagation(); onRemoveBranchLabel(i, bi); }}
                                ...
                                {branchStepCount > 0 && (
                                  <p className="text-[9px] text-muted-foreground pl-1">
                                    {t("flowRecorder.branchSteps", { count: branchStepCount, defaultValue: `${branchStepCount} step(s)` })}
                                  </p>
                                )}
                              </div>
                              );
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); onAddBranchLabel(i, t("flowRecorder.newBranch", "New branch")); }}
        ...
        {/* Add condition button — always visible when not already in a condition form */}
        {!conditionForm && (
          <button
            onClick={() => {
              setConditionForm({ index: -1, label: "", branches: ["", ""] });
            }}
        ...
        {conditionForm && (
          <div className="rounded-md border border-amber-400/30 bg-amber-500/5 p-2 space-y-1.5">
            <p className="text-[10px] font-semibold text-amber-400 uppercase">{t("flowRecorder.newCondition", "New Condition")}</p>
            <input
            ...
            {conditionForm.branches.map((b, bi) => (
              <div key={bi} className="flex items-center gap-1">
                ...
                {conditionForm.branches.length > 2 && (
                  <button onClick={() => setConditionForm({ ...conditionForm, branches: conditionForm.branches.filter((_, j) => j !== bi) })}
                    className="text-muted-foreground hover:text-destructive">
                    <X className="h-3 w-3" />
                  </button>
                )}
            ...
            <div className="flex gap-1 pt-1">
              <button onClick={handleConditionFormSubmit}
                className="flex-1 rounded bg-amber-500 px-2 py-1 text-[10px] font-semibold text-white hover:bg-amber-400 transition-colors">
                {t("flowRecorder.confirmCondition", "Confirm")}
              </button>
              <button onClick={() => setConditionForm(null)}
