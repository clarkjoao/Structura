...
import { getSafeWalkthroughVisual } from "../../utils/step-media.utils";
...
export function RightPanel({
  ...
  const persistMedia = useCallback(
    (mediaContent: NonNullable<WalkthroughStep["mediaContent"]>) => {
      if (!step) return;
      updateWalkthroughStep(walkthroughId, step.id, {
        mediaContent,
        svgContent: undefined,
      });
    },
    [walkthroughId, step, updateWalkthroughStep],
  );
    ...
    async (file: File) => {
      ...
      if (isRasterImageFile(file)) {
        try {
          ...
          toast.error(t("walkthroughs.editor.mediaReadError"));
        }
        return;
      }

      toast.error(t("walkthroughs.editor.invalidMediaFormat"));
    },
    [persistMedia, step, t],
  );
  ...
  const safeVisual = step ? getSafeWalkthroughVisual(step) : null;
    ...
    return (
      <div className="flex w-10 shrink-0 flex-col items-center gap-2 border-l border-border bg-card py-2">
        <Button
          ...
          size="icon"
          className="h-9 w-9"
          aria-label={t("walkthroughs.editor.expandVisualPanel")}
        ...
        <Button
          ...
          size="icon"
          className="h-9 w-9"
          aria-label={t("walkthroughs.editor.expandFlowPanel")}
  ...
  return (
    <div className="flex h-full min-h-0 w-[280px] shrink-0 flex-col overflow-hidden border-l border-border bg-card">
      <div className={visualSectionClassName}>
        <div className="flex h-11 shrink-0 items-center justify-between border-b border-border px-3">
          <span className="text-sm font-semibold text-foreground">
            {t("walkthroughs.editor.visualStateTitle")}
          </span>
          <Button
            type="button"
            variant="ghost"
            ...
            aria-label={
              visualOpen
                ? t("walkthroughs.editor.collapseVisualPanel")
                : t("walkthroughs.editor.expandVisualPanel")
            }
            ...
            {!step ? (
              ...
              <div className="grid gap-3">
                <button
                  ...
                  aria-label={isGlobalPlaying ? t("walkthroughs.editor.expandVisual") : undefined}
                >
                  {safeVisual?.kind === "svg" ? (
                    <div
                    ...
                    <img
                      src={safeVisual.src}
                      alt={t("walkthroughs.editor.mediaPreviewAlt")}
                  ...
                  {isGlobalPlaying ? (
                    <div className="absolute inset-0 flex items-center justify-center bg-background/0 opacity-0 transition-all group-hover:bg-background/40 group-hover:opacity-100">
                      <span className="rounded-full bg-card px-3 py-1 text-xs font-medium text-foreground shadow">
                        {t("walkthroughs.editor.expandVisual")}
                      </span>
                    </div>
                  ) : null}
                </button>
                {!isGlobalPlaying ? (
                  ...
                  >
                    ...
                    {t("walkthroughs.editor.removeMedia")}
                  </Button>
                ) : null}
              </div>
            ) : isGlobalPlaying ? (
              <p className="text-center text-sm text-muted-foreground">
                {t("walkthroughs.editor.noVisualForStep")}
              </p>
            ) : (
              <div
                tabIndex={0}
                ...
                role="group"
                aria-label={t("walkthroughs.editor.uploadMedia")}
              >
                <label
                  htmlFor={fileInputId}
                ...
                >
                  ...
                  <span className="text-center text-sm text-muted-foreground">
                    {t("walkthroughs.editor.uploadMedia")}
                  </span>
                  <input
                    ref={fileInputRef}
                    id={fileInputId}
                ...
                <p className="text-center text-xs text-muted-foreground">
                  {t("walkthroughs.editor.pasteMedia")}
                </p>
              </div>
            )}
          </div>
      ...
      <div className={flowSectionClassName}>
        ...
        >
          <span className="text-sm font-semibold text-foreground">
            {t("walkthroughs.editor.flowSectionTitle")}
          </span>
          <Button
            type="button"
            variant="ghost"
            ...
            aria-label={
              flowOpen
                ? t("walkthroughs.editor.collapseFlowPanel")
                : t("walkthroughs.editor.expandFlowPanel")
            }
        ...
        {flowOpen ? (
          <div className="min-h-0 flex-1 overflow-y-auto p-3">
            {isGlobalPlaying ? (
              <p className="text-center text-xs text-muted-foreground">
                {t("walkthroughs.editor.flowLockedDuringPlayback")}
              </p>
            ) : (
              flowSection
            )}
