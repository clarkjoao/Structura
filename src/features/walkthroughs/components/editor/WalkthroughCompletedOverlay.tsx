...
interface WalkthroughCompletedOverlayProps {
  walkthroughName: string;
  onRestart: () => void;
  onExit: () => void;
}

export function WalkthroughCompletedOverlay({
  walkthroughName,
  onRestart,
  onExit,
}: WalkthroughCompletedOverlayProps) {
    ...
    <div className="pointer-events-auto absolute inset-0 z-30 flex items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="flex max-w-md flex-col items-center gap-6 rounded-2xl border border-border bg-card p-10 shadow-2xl">
        ...
        <div className="text-center">
          <p className="text-lg font-semibold text-foreground">{walkthroughName}</p>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("walkthroughs.presentation.completed")}
          </p>
        </div>
        <div className="flex flex-wrap justify-center gap-3">
          <Button type="button" variant="outline" onClick={onRestart}>
            {t("walkthroughs.presentation.restart")}
          </Button>
          <Button type="button" variant="secondary" onClick={onExit}>
            {t("walkthroughs.presentation.exitPresentation")}
          </Button>
        </div>
      </div>
    </div>
