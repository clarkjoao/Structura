...
export function VisualStateOverlay({ step, onClose }: VisualStateOverlayProps) {
  ...
  const visual = useMemo(() => getSafeWalkthroughVisual(step), [step]);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("walkthroughs.editor.visualOverlayLabel")}
    >
      <div
      ...
      >
        <button
          ...
          onClick={onClose}
          aria-label={t("walkthroughs.editor.closeVisual")}
        >
          <X className="h-4 w-4" />
        </button>
