...
const walkthroughEdgeTypes = { editable: EditableEdge, smoothstep: SmoothStepEdge };
...
interface WalkthroughEditorCanvasProps {
  diagramId: string | null;

  hasSelectedStep: boolean;
  isGlobalPlaying?: boolean;
  showVisualOverlay?: boolean;
  selectedStep?: WalkthroughStep | null;
...
type WalkthroughEditorCanvasInnerProps = Omit<
  WalkthroughEditorCanvasProps,
  | "hasSelectedStep"
  | "showVisualOverlay"
  | "selectedStep"
...
interface WalkthroughEditorPlaybackFitViewProps {
  diagramId: string;
  isPlaying: boolean;
  isRecording: boolean;
...
function WalkthroughEditorPlaybackFitView({
  diagramId,
  isPlaying,
  isRecording,
  activeFlow,
  currentStepId,
}: WalkthroughEditorPlaybackFitViewProps): null {
...
interface WalkthroughEditorDiagramFitViewProps {
  diagramId: string;
}

function WalkthroughEditorDiagramFitView({
  diagramId,
}: WalkthroughEditorDiagramFitViewProps): null {
...
function WalkthroughEditorNoStepEmptyState() {
  ...
  return (
    ...
    >
      ...
      <p className="max-w-sm text-base font-medium text-muted-foreground">
        {t("walkthroughs.editor.emptyStateTitle")}
      </p>
      <p className="max-w-sm text-sm text-muted-foreground/70">
        {t("walkthroughs.editor.emptyStateSubtitle")}
      </p>
    </div>
  );
}

function WalkthroughEditorCanvasInner({
  diagramId,
  isGlobalPlaying = false,
}: WalkthroughEditorCanvasInnerProps) {
  const { t } = useTranslation();
  ...
  const flowVisuals = useMemo((): WalkthroughEditorCanvasFlowVisuals | null => {
  ...
  const nodes = useMemo(() => {
    if (!diagram) return [];
    return buildWalkthroughEditorNodes(diagram, null, diagramsRecord, flowVisuals);
  }, [diagramsRecord, diagram, flowVisuals]);

  const edges = useMemo(() => {
    if (!diagram) return [];
    return buildWalkthroughEditorEdges(diagram, flowVisuals);
  }, [diagram, flowVisuals]);
  ...
  if (!diagramId || !diagram) {
    return (
      <div className="flex h-full w-full items-center justify-center bg-muted/20">
        <p className="text-sm text-muted-foreground">{t("walkthroughs.editor.selectDiagram")}</p>
      </div>
    );
  }
  ...
  const topCanvasHint = showFlowNavigator
    ...
    : showRecordingOverlay
      ? t("walkthroughs.editor.recordingCanvasHint")
      : null;
  ...
  return (
    <div className="relative h-full w-full">
      <ReactFlow
        ...
        edges={edges}
        nodeTypes={nodeTypes}
        edgeTypes={walkthroughEdgeTypes}
      ...
      >
        <WalkthroughEditorDiagramFitView diagramId={diagramId} />
        <WalkthroughEditorPlaybackFitView
          diagramId={diagramId}
          isPlaying={isPlaying}
      ...
      {isGlobalPlaying ? (
        <div className="pointer-events-none absolute bottom-14 left-1/2 z-10 -translate-x-1/2">
          <div className="rounded-full border border-border bg-card/80 px-3 py-1 text-[10px] text-muted-foreground backdrop-blur-sm">
            {t("walkthroughs.presentation.keyboardHint")}
          </div>
        </div>
      ) : null}
    </div>
...
export function WalkthroughEditorCanvas({
  hasSelectedStep,
  ...
  if (!hasSelectedStep) {
    return <WalkthroughEditorNoStepEmptyState />;
  }
  return (
    <div className="relative h-full w-full">
      <ReactFlowProvider>
        <WalkthroughEditorCanvasInner {...innerProps} />
      </ReactFlowProvider>
